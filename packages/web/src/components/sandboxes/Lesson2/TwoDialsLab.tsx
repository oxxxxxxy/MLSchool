...
export const TwoDialsLab: React.FC = () => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [k, setK] = useState<number>(1.7);
  ...
  useEffect(() => {
    ...
    // Intersection with Y axis (x = 0, y = b)
    const yInterceptPx = centerX;
    const yInterceptPy = centerY - b * scale;

    ctx.fillStyle = '#d29922';
    ctx.beginPath();
    ctx.arc(yInterceptPx, yInterceptPy, 6, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.fillText(`b = ${b.toFixed(1)}`, yInterceptPx + 10, yInterceptPy + 4);
  ...
  return (
    <div className="p-4 sm:p-5 rounded-xl bg-[#161b22] border border-[#30363d] space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <span className="text-[11px] font-mono text-[#58a6ff] uppercase">Интерактив: Два пульта управления</span>
          <h3 className="text-sm font-semibold text-[#c9d1d9]">Два числа полностью управляют прямой</h3>
        </div>
      </div>
        ...
        <div className="flex-1 w-full space-y-3">
          <div className="p-3 rounded bg-[#0d1117] border border-[#30363d] space-y-1">
            <div className="flex justify-between text-xs font-mono">
              <span className="text-[#8b949e]">Пульт 1: Наклон (k):</span>
          ...
          <div className="p-3 rounded bg-[#0d1117] border border-[#30363d] space-y-1">
            <div className="flex justify-between text-xs font-mono">
              <span className="text-[#8b949e]">Пульт 2: Сдвиг по вертикали (b):</span>
          ...
          <div className="p-2.5 rounded bg-[#0d1117] border border-[#30363d] text-center font-mono text-sm">
            <span className="text-[#8b949e]">y = </span>
            <span className="text-[#58a6ff] font-bold">{k.toFixed(1)}x </span>
            <span className="text-[#d29922] font-bold">{b >= 0 ? `+ ${b.toFixed(1)}` : `- ${Math.abs(b).toFixed(1)}`}</span>
          </div>
        </div>
      </div>
    </div>
